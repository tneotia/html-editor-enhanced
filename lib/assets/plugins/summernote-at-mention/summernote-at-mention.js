...
(function(factory) {
  console.error("factory(at-mention)");
  ...
  $.extend($.summernote.plugins, {
    summernoteAtMention: function(context) {
      console.error("Adding summernoteAtMention");
      ...
      this.events = {
        ...
        "summernote.keydown": (_, event) => {
          console.error("summernote.keydown.keycode = " + event.keyCode);
          if (this.showingAutocomplete) {
            switch (event.keyCode) {
              case ENTER_KEY_CODE: {
                console.error("before preventDefault(ENTER_KEY_CODE)");
                event.preventDefault();
                console.error("before stopPropagation(ENTER_KEY_CODE)");
                event.stopPropagation();
                this.handleEnter();
                break;
              }
              case UP_KEY_CODE: {
                console.error("before preventDefault(UP_KEY_CODE)");
                event.preventDefault();
                console.error("before stopPropagation(UP_KEY_CODE)");
                event.stopPropagation();
                const newIndex =
              ...
              case DOWN_KEY_CODE: {
                console.error("before preventDefault(DOWN_KEY_CODE)");
                event.preventDefault();
                console.error("before stopPropagation(DOWN_KEY_CODE)");
                event.stopPropagation();
                const newIndex =
      ...
      this.showAutocomplete = (atTextIndex, indexAnchor) => {
        console.error("atTextIndex = " + atTextIndex + ", indexAnchor = " + indexAnchor);
        if (this.showingAutocomplete) {
          throw new Error(
